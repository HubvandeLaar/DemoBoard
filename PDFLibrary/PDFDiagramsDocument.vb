...
Public Class PDFDiagramsDocument

    Private gPDFSharpDocument As New Pdf.PdfDocument()

    Public Property PageNumber As Integer = 0
    Public Property DiagramsPerPage As Integer = 12 ', 9, 6 or 4
    Public Property PageMarge As Long = CentimetersToPoints(1.5)   'cm
    Public Property ColumnMarge As Long = CentimetersToPoints(0.5) 'cm
    Public Property HeaderHeight As Long = CentimetersToPoints(1.5)   'cm
    Public Property FooterMarge As Long = CentimetersToPoints(1.5) 'cm

    Public Property PageHeader As String
        Set(pPageHeader As String)
            gPDFSharpDocument.Info.Title = pPageHeader
        End Set
        Get
            Return gPDFSharpDocument.Info.Title
        End Get
    End Property
    ...
    Sub New()
        gPDFSharpDocument.Info.Title = ""
        gPDFSharpDocument.Info.Creator = "DemoBoard PDF Export"
        gPDFSharpDocument.Info.CreationDate = Now()
    End Sub

    Private Sub InsertPageHeader(pPageHeader As String, pGraphics As XGraphics)
        Dim HeaderRect As New XRect(New XPoint(PageMarge, PageMarge), New XSize(pGraphics.PdfPage.Width.Point - (PageMarge * 2), PageMarge + HeaderHeight))
        Dim Formatter As New XTextFormatter(pGraphics)
        Formatter.DrawString(pPageHeader, New XFont("Calibri", 24), XBrushes.Black, HeaderRect, XStringFormats.TopLeft)
        'And PageNumber
        PageNumber += 1
        Dim PageNumberRect As New XRect(New XPoint(PageMarge, pGraphics.PdfPage.Height.Point - PageMarge), New XSize(pGraphics.PdfPage.Width.Point - (PageMarge * 2), PageMarge))
        Formatter.DrawString(Format(PageNumber), New XFont("Calibri", 12), XBrushes.Black, PageNumberRect, XStringFormats.TopLeft)
    End Sub
    ...
    Sub InsertDiagram(pGameIndex As Long, pDiagram As Bitmap, pBottomText As String, pFontSize As Double)
        ...
        If pGameIndex > (DiagramsPerPage * gPDFSharpDocument.PageCount) Then
            CurPage = gPDFSharpDocument.Pages.Add()
            Graphics = XGraphics.FromPdfPage(CurPage)
            InsertPageHeader(PageHeader, Graphics)
        Else
            CurPage = gPDFSharpDocument.Pages(gPDFSharpDocument.PageCount - 1)
            Graphics = XGraphics.FromPdfPage(CurPage)
        End If
        ...
        'Draw Bottom Text
        Dim TextRect As New XRect(Exercise.Left, Exercise.Top + DiagramRect.Height, Exercise.Width, Exercise.Height - DiagramRect.Height + ColumnMarge)
        Dim Formatter As New XTextFormatter(Graphics)
    ...
    Sub Save(pFileName As String)
        gPDFSharpDocument.Save(pFileName)
    End Sub

    ''' <summary>Returns a distance in Points for a specified distance in Centimeters</summary>
    Public Function CentimetersToPoints(pCentimeters As Single) As Long
        Return pCentimeters * 28.35
    End Function

    Protected Overrides Sub Finalize()
        Me.gPDFSharpDocument = Nothing
